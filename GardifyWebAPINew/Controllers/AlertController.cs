...
namespace GardifyWebAPI.Controllers
{
	public class AlertController : _BaseController
	{
		...
		//	WeatherForecast forecast = await wh.getWeatherForecastByGeoCoords(51.5105434f, 7.4567403f);
		...
		public ActionResult EditAlert(AlertViewModels.AlertViewModel newData)
		{
            ...
            throw new Exception("Seite konnte nicht gefunden werden.");//, HttpStatusCode.NotFound, "AlertController.EditAlert("+newData.Title+")");
        }
		...
		public ActionResult DeleteAlert(int id, string returnUrl)
		{
            ...
            throw new Exception("Seite konnte nicht gefunden werden.");//, HttpStatusCode.NotFound, "AlertController.DeleteAlert("+id+","+ returnUrl+")"); 
        }
		...
		public HelperClasses.DbResponse DbCreateAlert(Alert newAlert)
		{
			...
			plantDB.AlertTrigger.Add(defaultTrigger);
		...
		{
			Alert alert = (from a in plantDB.Alert
		...
		{
			Alert alertToEdit = (from a in plantDB.Alert
		...
		{
			var alert_sel = (from a in plantDB.Alert.Include(a => a.Triggers.Select(t => t.Conditions))
						   where !a.Deleted && a.RelatedObjectId == objectId && a.ObjectType == objectType
						 select a);

			//var alert_sel = plantDB.Alert.Include("Triggers.Conditions").Where(a => !a.Deleted && a.RelatedObjectId == objectId && a.ObjectType == objectType);
		...
		[NonAction]
		public IEnumerable<Alert> DbGetTriggeredAlertsByPlantId(int plantId, GardifyModels.Models.WeatherHelpers.WeeklyForecastsObj forecast, float frostBuffer = 0)
		{
			...
			if (plantAlerts != null && plantAlerts.Any())
			{
				...
				{
					bool isTriggered = DbCheckAlert(alert, forecast, frostBuffer);
					if (isTriggered)
					{
		...
		public bool DbCreateAlertTrigger(AlertTrigger alertTriggerData)
		{
			...
			plantDB.AlertTrigger.Add(alertTriggerData);
			return plantDB.SaveChanges() > 0 ? true : false;
		}
		...
		{
			IEnumerable<AlertTrigger> triggers = (from at in plantDB.AlertTrigger
		...
		public HelperClasses.DbResponse DbCreateAlertCondition(AlertCondition alertConditionData)
		{
			...
			plantDB.AlertCondition.Add(alertConditionData);
		...
		{
			AlertCondition cond = (from c in plantDB.AlertCondition
		...
		{
			var trigger_sel = (from ac in plantDB.AlertCondition
		...
		[NonAction]
		public bool DbCheckAlert(Alert alert, GardifyModels.Models.WeatherHelpers.WeeklyForecastsObj forecast, float frostBuffer = 0)
		{
			bool ret = false;
			...
			if (alert.Triggers != null && alert.Triggers.Any())
			{
				...
				foreach (AlertTrigger trigger in alert.Triggers)
				{
					bool triggerResult = DbCheckTrigger(trigger, forecast, frostBuffer);
		...
		[NonAction]
		public bool DbCheckTrigger(AlertTrigger alertTrigger, GardifyModels.Models.WeatherHelpers.WeeklyForecastsObj forecast, float frostBuffer = 0)
		{
			bool ret = false;
			...
			if (alertTrigger.Conditions != null && alertTrigger.Conditions.Any())
			{
				...
				{
					bool conditionResult = DbCheckCondition(condition, forecast, frostBuffer);
        ...
        [NonAction]
        public bool DbCheckCondition(AlertCondition condition, GardifyModels.Models.WeatherHelpers.WeeklyForecastsObj forecast, float frostBuffer = 0)
        {
            bool ret = false;
            ...
            if (condition.DateValue != null)
            {
                ...
                if (condition.FloatValue != null)
                {
                   ...
                   condition.ValueType == ModelEnums.ComparedValueType.YearsSpan)
                    {
                        return true;    // ignore DateSpans: doesnt matter when the frost happens
                        double timeSpan = 0;
            ...
            if (condition.FloatValue != null)
            {
                    ...
                    condition.ValueType == ModelEnums.ComparedValueType.MinWindSpeed || condition.ValueType == ModelEnums.ComparedValueType.MaxWindSpeed)
                {
                    if (forecast != null)
                    {
                        ...
                        {
                            case ModelEnums.ComparedValueType.MinWindSpeed:
                                valueToCompare = forecast.Forecasts.Min(f => f.maxWindSpeedInKilometerPerHour);
                                break;
                            case ModelEnums.ComparedValueType.MaxWindSpeed:
                                valueToCompare = forecast.Forecasts.Min(f => f.maxWindSpeedInKilometerPerHour);
                                break;
                            case ModelEnums.ComparedValueType.MinTemperature:
                                valueToCompare = forecast.Forecasts.Min(f => f.MinAirTemperatureInCelsius) - frostBuffer;
                                break;
                            case ModelEnums.ComparedValueType.MaxTemperature:
                                valueToCompare = forecast.Forecasts.Min(f => f.MaxAirTemperatureInCelsius);
                                break;
                            default:
                                break;
                        }
