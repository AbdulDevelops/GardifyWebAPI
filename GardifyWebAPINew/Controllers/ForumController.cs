...
namespace GardifyWebAPI.Controllers
{
    public class ForumController : _BaseController
    {
		...
		public ForumViewModels.ForumIndexViewModel Index(int? taxonId)
		{
			...
			{
				IEnumerable<ForumHeader> plantForumHeaders = plantDB.ForumHeaders.Where(f => f.IsThread);
		...
		public ForumViewModels.ForumThreadViewModel Thread(int? id)
		{
			if(id != null)
			{
            ...
            throw new Exception("Seite konnte nicht gefunden werden.");//, HttpStatusCode.NotFound, "ForumController.Thread("+id+")");
        }

        public ForumViewModels.ForumThreadViewModel PlantThread(int plantId)
        {
			ForumViewModels.ForumThreadViewModel viewModel = new ForumViewModels.ForumThreadViewModel();
			...
			{
                throw new Exception("Seite konnte nicht gefunden werden.");//, HttpStatusCode.NotFound, "ForumController.PlantThread("+plantId+")");
            }
        ...
        public ForumViewModels.ForumThreadViewModel CreatePlantPost(ForumViewModels.ForumPostViewModel newPostData, int plantId)
        {
           if(ModelState.IsValid)
			{
                //Guid userId = Guid.Parse(User.Identity.GetUserId());
                //string userName = User.Identity.GetUserName();
                //Guid userId = new Guid("a54c66bc-6dd4-4b78-b6d8-214f6ab76e9f");
                var userId = Utilities.GetUserId();

                string userName = "test_author";
		...
		{			
			if (header != null)
			{
				...
				return new ForumViewModels.ForumHeaderViewModel
				{
					...
					Title = header.Title,
                    PostCount = header.RelatedPosts.Count()
				};
			}
			return null;
		}
		...
		public bool DbCreateForumHeader(ForumHeader newForumHeader)
		{
			...
			plantDB.ForumHeaders.Add(newForumHeader);
		...
		{
			return (from h in plantDB.ForumHeaders
		...
		{
			return (from p in plantDB.ForumPost
